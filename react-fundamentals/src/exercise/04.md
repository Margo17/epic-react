...
PropTypes module is deprecated by the current wide usage of TypeScript as it is
superior in checking type validity during runtime. PropTypes implementation is
pretty trivial though as a type check function consists of a simple if statement
(depends on the type checked) and you can simply write the type check yourself.
