...
React actually uses synthetic events for performance reasons. Synthetic events
are not native events, but have a very similar structure. You can access native
event `event.nativeEvent`. For getting input values just access synthetic event
`event.target.elements.inputName.value` or `event.target.elements[0].value`.
Also controlled inputs are the ones who have value prop. And for initial value
you can pass defaultValue prop.

Also what I've remembered is that it's better to use console.dir() to view
object structure instead of console.log(), let's abuse it then!
