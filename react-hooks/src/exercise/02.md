...
Main component lifecycle parts are these: first component is **mounted** and
runs lazy initializers, then if **update** happens react does these things:

_Renders component => Updates DOM => Cleanup layout effects => Run layout
effects => Browser paints screen => Cleanup effects => Run side effects_

! Children side effects happen before parent's, keep that in mind.
